...
def call_imink(id_token, timestamp, request_id):
    # def call_flapg(id_token, timestamp, request_id, hash, type):
    # Calls the imink API to get an "f-code" for a login request
    # this is generated by the NSO app but hasn't been reverse-engineered at the moment.
    # -----
    # flapg api is "404 Not found".
    # Therefore, updated to use imink f-api.

    imink_resp = rsess.post("https://api.imink.app/f", json={
    ...
    if imink_resp.status_code != 200:
        print("Error obtaining f-code from imink API, aborting... ({})".format(imink_resp.text))

    return imink_resp.json()["f"], imink_resp.json()["request_id"]
...
def login_switch_web(id_token, nintendo_profile):
    ...
    nso_f, registrationToken = call_imink(id_token, timestamp, request_id)
